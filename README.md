...
- Несикабельный поток (сеть, pipe) читается частями через пул буферов (`ArrayPool`), память ограничена размером одной части; короткие потоки сворачиваются в обычный `PutObject`. Для очень длинных потоков размер части удваивается каждые 1000 частей (до 512 МБ), так что в лимит 10 000 частей укладывается ~3 ТБ.
